...
$('clearManual').addEventListener('click', ()=>{manualSpots=[]; render(); updateResults(summary());
  try{ updateComparePanel(); }catch(e){ console.warn(e); }
  try{ updateComparePanel(); }catch(e){ console.warn(e); }});
...
function updateComparePanel(){
  ...
  if(!before || !after || !wrap || !slider) return;

  // Before = current annotated detection map.
  render();
  before.src = canvas.toDataURL('image/png');

  // After = clean preview simulation.
  if(!cleanData) createCleanPreview();
  after.src = imageDataToDataUrl(cleanData);
  ...
  window.addEventListener('load', function(){
    ...
    if(slider && !slider.dataset.boundCompare){
      slider.addEventListener('input', function(){
        const wrap=document.getElementById('compareAfterWrap');
        if(wrap) wrap.style.width = (Number(slider.value || 50)) + '%';
      });
      slider.dataset.boundCompare='1';
    }
  });
