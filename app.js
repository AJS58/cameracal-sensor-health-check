...
function getDustOverlayColour(){
  ...
  return hidden ? hidden.value : '#ff3030';
}
...
function runDetection(){
  ...
  const detectedComponents = connectedComponents(closed,w,h,minArea, w*h).map(c=>({
    x:c.cx,
    y:c.cy,
...
(function(){
  ...
  document.querySelectorAll('.colourChoice').forEach(btn => {
    btn.addEventListener('click', function(){
      if(hidden) hidden.value = this.dataset.colour || '#ff3030';
