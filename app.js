...
function generateReportWindow(){
  ...
  const confidenceDry = (noDetection || lightDetection) ? 0 : Math.max(5, Math.round(100 - confidenceOrganic/1.4));
  ...
  ` : lightDetection ? `
    <div class="cta"><h3>Very light / isolated contamination detected</h3><p>${s.count} isolated mark${s.count===1?' was':'s were'} detected from the supplied image.</p><p>The level detected is too low to produce a reliable contamination-type classification, so dust / bonded debris / oily residue confidence percentages have been suppressed.</p><p>Recommended action: blower check, monitor, or re-test using a controlled f/16–f/22 dust-test image.</p></div>
  ...
  ` : `
    ...
    <div class="grid3"><div class="card">Dry dust / particles<strong>${confidenceDry}%</strong><div class="bar"><span style="width:${confidenceDry}%"></span></div></div><div class="card">Organic / bonded debris<strong>${confidenceOrganic}%</strong><div class="bar"><span style="width:${confidenceOrganic}%"></span></div></div><div class="card">Moisture / oily residue<strong>${confidenceMoisture}%</strong><div class="bar"><span style="width:${confidenceMoisture}%"></span></div></div></div>
