...
$('clearManual').addEventListener('click', ()=>{manualSpots=[]; render(); updateResults(summary());
  try{ initialiseCompareSlider(); }catch(e){}});
...
function initialiseCompareSlider(){
  const slider=document.getElementById('compareSlider');
  const after=document.getElementById('compareAfter');
  const wrap=document.querySelector('.compareWrap');

  if(!slider || !after || !wrap) return;
  ...
  // Touch support
  slider.addEventListener('touchmove', update,{passive:true});
...
(function(){
  window.addEventListener('load', () => {
    try{ initialiseCompareSlider(); }catch(e){ console.log(e); }
  });
})();
