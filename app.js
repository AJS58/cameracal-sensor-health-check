...
let selectedHighlightColour = '#ff3030';
...
function loadImageToCanvas(){
  ...
  cleanData = null; spots=[]; oilSpots=[]; manualSpots=[]; scale=1; cleanMode=false; overlay=true;
...
const hc=$('highlightColour'); if(hc) hc.addEventListener('change', render);
...
function sampleCircle(gray,w,h,cx,cy,r,n=24){
  const vals=[];
  for(let k=0;k<n;k++){
...
function detectOilGreaseRings(gray,w,h){
  // Detect circular halo/ring or "fried egg" patterns that can be consistent
  // with oil, grease or shutter/mirror mechanism lubricant.
  const candidates=[];
  const step=Math.max(2, Math.round(Math.min(w,h)/650));
  const minR=Math.max(4, Math.round(Math.min(w,h)/430));
  const maxR=Math.max(10, Math.round(Math.min(w,h)/90));

  for(let y=maxR+2;y<h-maxR-2;y+=step){
    for(let x=maxR+2;x<w-maxR-2;x+=step){
      const centre=gray[y*w+x];
      for(let r=minR;r<=maxR;r+=Math.max(2, Math.round(r/3))){
        const inner=medianOf(sampleCircle(gray,w,h,x,y,Math.max(2,Math.round(r*0.45)),16));
        const ring=medianOf(sampleCircle(gray,w,h,x,y,r,28));
        const outer=medianOf(sampleCircle(gray,w,h,x,y,Math.round(r*1.65),32));
        const centreDark=inner-centre;
        const ringContrast=Math.abs(ring-outer);
        const score=(centreDark*0.65)+(ringContrast*0.9);
        if(centreDark>5 && ringContrast>4 && score>11){
          candidates.push({x,y,r:Math.max(7,r*1.35),area:Math.PI*r*r,bw:r*2,bh:r*2,oil:true,grit:false,oilScore:score});
          break;
        }
      }
    }
  ...
  candidates.sort((a,b)=>b.oilScore-a.oilScore);
  const final=[];
  for(const c of candidates){
    const duplicate=final.some(o=>Math.hypot(o.x-c.x,o.y-c.y)<Math.max(o.r,c.r)*0.9);
    if(!duplicate) final.push(c);
    if(final.length>160) break;
  }
  return final;
}

function componentContrast(gray,w,h,c){
  const maxDim=Math.max(c.bw||1,c.bh||1);
  const rOuter=Math.max(8,Math.round(maxDim*1.8));
  const rInner=Math.max(2,Math.round(maxDim*0.35));
  const cx=Math.round(c.cx), cy=Math.round(c.cy);
  const innerVals=[], outerVals=[], nearVals=[];
  for(let yy=Math.max(0,cy-rOuter); yy<=Math.min(h-1,cy+rOuter); yy++){
    for(let xx=Math.max(0,cx-rOuter); xx<=Math.min(w-1,cx+rOuter); xx++){
      const d=Math.hypot(xx-cx,yy-cy);
      const v=gray[yy*w+xx];
      if(d<=rInner) innerVals.push(v);
      else if(d>=rOuter*0.65 && d<=rOuter) outerVals.push(v);
      if(d<=rOuter) nearVals.push(v);
    }
  }
  const inner=medianOf(innerVals);
  const outer=medianOf(outerVals);
  const minv=nearVals.length?Math.min(...nearVals):inner;
  const maxv=nearVals.length?Math.max(...nearVals):outer;
  return {darkContrast:outer-inner, localRange:maxv-minv};
}

function isGritLikeComponent(c,gray,w,h){
  // Grit/sand/granular debris tends to be larger, hard edged, high contrast and irregular.
  ...
  const cc=componentContrast(gray,w,h,c);

  if(c.area < 45) return false;
  if(maxDim < 6 || maxDim > Math.min(w,h)*0.09) return false;
  if(aspect > 4.2) return false;

  const hardParticle = cc.darkContrast > 18 && cc.localRange > 34;
  const granularShape = compactness > 0.10 && compactness < 0.82;
  const largeIrregular = c.area > 180 && compactness < 0.72 && cc.darkContrast > 12;

  return (hardParticle && granularShape) || largeIrregular;
}

function removeOverlaps(base, blockers, factor=0.78){
  return base.filter(d => !blockers.some(o => Math.hypot(o.x-d.x,o.y-d.y) < Math.max(o.r,d.r)*factor));
}

function getDustOverlayColour(){
  const el=document.getElementById('highlightColour');
  return el ? el.value : '#ff3030';
}
...
function runDetection(){
  ...
  const baseSpots = connectedComponents(closed,w,h,minArea, w*h).map(c=>({
    x:c.cx,
    y:c.cy,
  ...
  oilSpots = detectOilGreaseRings(gray,w,h);
  gritSpots = baseSpots
    .filter(s => isGritLikeComponent(s.raw, gray, w, h))
    .map(s => ({...s, grit:true, oil:false, r:Math.max(s.r, 8)}));

  spots = removeOverlaps(baseSpots.filter(s=>!isGritLikeComponent(s.raw, gray, w, h)), [...oilSpots,...gritSpots], 0.75);
  render(); updateResults(summary());
}
...
function summary(){
  ...
  else if(count>15||heavy>0||gritCount>0||oilCount>1){sev='Medium';rec=gritCount>0?'Professional inspection recommended — hard particulate suspected':'Professional inspection / wet clean may be required';}
  ...
  const visibleImpact = count > 0 ? Math.max(1, Math.min(100, Math.round((physicalCoverage * 4.2) + densityImpact + (heavy * 1.8) + (oilCount * 1.6) + (gritCount * 3.0)))) : 0;
  ...
  return {count,small,medium,large:heavy,oilCount,gritCount,sev,rec,pattern,coverage,physicalCoverage,healthScore,meaningfulDetection};
...
function render(){
  ...
  if(overlay){
    ...
    [...spots,...manualSpots].forEach((s,i)=>{ctx.strokeStyle=dustColour;ctx.fillStyle=dustColour+'16';ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fill();ctx.stroke(); if(i<80){ctx.fillStyle=dustColour;ctx.font=`${Math.max(12,canvas.width/120)}px Arial`;ctx.fillText(String(i+1),s.x+s.r+3,s.y);}});
    oilSpots.forEach((s,i)=>{ctx.strokeStyle='#ff9900';ctx.fillStyle='rgba(255,153,0,.12)';ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.beginPath();ctx.arc(s.x,s.y,Math.max(3,s.r*.45),0,Math.PI*2);ctx.stroke(); if(i<80){ctx.fillStyle='#ff9900';ctx.font=`${Math.max(12,canvas.width/120)}px Arial`;ctx.fillText('O'+String(i+1),s.x+s.r+3,s.y);}});
    gritSpots.forEach((s,i)=>{ctx.strokeStyle='#7e2cff';ctx.fillStyle='rgba(126,44,255,.12)';ctx.beginPath();ctx.rect(s.x-s.r,s.y-s.r,s.r*2,s.r*2);ctx.fill();ctx.stroke(); if(i<80){ctx.fillStyle='#7e2cff';ctx.font=`${Math.max(12,canvas.width/120)}px Arial`;ctx.fillText('G'+String(i+1),s.x+s.r+3,s.y);}});
    ctx.restore();
  }
}
function fitCanvas(){
...
function generateReportWindow(){
  ...
  const confidenceMoisture = (noDetection || lightDetection) ? 0 : Math.min(88, Math.round((s.large*3) + (s.coverage*5) + ((s.pattern.includes('moisture')||s.pattern.includes('oil'))?35:20)));
  ...
  ` : lightDetection ? `
    <div class="cta"><h3>Very light / isolated contamination detected</h3><p>${s.count} isolated mark${s.count===1?' was':'s were'} detected from the supplied image.</p><p>The level detected is too low to produce a reliable contamination-type classification, so dust / oil / grease / bonded debris confidence percentages have been suppressed.</p><p>Recommended action: blower check, monitor, or re-test using a controlled f/16–f/22 dust-test image.</p></div>
  ...
  ` : `
    <p>Where a contamination type is suggested, it is an indicative pattern only. Physical inspection may be required for confirmation, especially with suspected bonded debris, organic residue, oil / grease / shutter lubricant, or moisture-related contamination.</p><p>${(s.oilCount||0)>0 ? 'Circular halo or ring-type marks have been detected. This pattern can be consistent with oil, grease or shutter/mirror mechanism lubricant contamination, but physical inspection is required for confirmation.' : ''}</p><p>${(s.oilCount||0)>0 ? 'Circular halo or ring-type marks have been detected. This pattern can be consistent with oil, grease or shutter/mirror mechanism lubricant contamination, but physical inspection is required for confirmation.' : ''}</p><p>${(s.gritCount||0)>0 ? 'Larger high-contrast irregular particles have been detected. This may be consistent with grit, sand or granular debris. Avoid contact cleaning if hard particles are suspected and seek professional inspection.' : ''}</p>
    <div class="grid4"><div class="card">Dry dust / particles<strong>${confidenceDry}%</strong><div class="bar"><span style="width:${confidenceDry}%"></span></div></div><div class="card">Oil / grease / lubricant<strong>${confidenceOil}%</strong><div class="bar"><span style="width:${confidenceOil}%"></span></div></div><div class="card">Organic / bonded debris<strong>${confidenceOrganic}%</strong><div class="bar"><span style="width:${confidenceOrganic}%"></span></div></div><div class="card">Moisture / oily residue<strong>${confidenceMoisture}%</strong><div class="bar"><span style="width:${confidenceMoisture}%"></span></div></div></div>
  ...
  const html=`<!doctype html><html><head><title>Cameracal Sensor Health Report</title><style>
    ...
    .grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:18px 0}.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:18px 0}.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:18px 0}.grid2{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:18px 0}
  ...
  <div class="page"><div class="head"><div><h1>Sensor Health Check Report</h1><p><b>Cameracal Services – The Camera Specialist</b><br>Dust Verification & Analysis Report</p></div><img src="${logoUrl}" alt="Cameracal Services"></div><p><b>Report date:</b> ${new Date().toLocaleString()}<br><b>Image:</b> ${currentFileName||'Uploaded image'}<br><b>Report ID:</b> CS-${Date.now()}</p><div class="grid"><div class="card">Total spots<strong>${s.count}</strong></div><div class="card">Severity<strong class="${s.sev==='Extreme'?'red':''}">${s.sev}</strong></div><div class="card">Visible contamination impact<strong>${s.coverage.toFixed(0)}%</strong></div><div class="card">Health score<strong>${s.healthScore}/100</strong></div></div><p>This report analyses sensor contamination visible under dust-revealing conditions. Results depend on the supplied image and shooting conditions.</p><p class="small"><b>Visible Contamination Impact</b> is a practical severity estimate based on spot count, spot size, clustering and heavy contamination. It is not a literal measurement of physical sensor area covered.</p></div>

  <div class="page"><h2>Dust Map & Distribution</h2><img class="map" src="${imgUrl}"><div class="grid"><div class="card">Small<strong>${s.small}</strong><span class="small">1–4px</span></div><div class="card">Medium<strong>${s.medium}</strong><span class="small">5–15px</span></div><div class="card">Large / Heavy<strong>${s.large}</strong><span class="small">&gt;15px</span></div><div class="card">Oil / grease indicators<strong>${s.oilCount||0}</strong><span class="small">halo/ring marks</span></div><div class="card">Visible impact<strong>${s.coverage.toFixed(0)}%</strong></div></div></div>

  <div class="page"><h2>Contamination Heat Map</h2><img class="map" src="${heatUrl}"><div class="heatLegend"><div class="heatItem"><span class="heatSwatch" style="background:#ff0000"></span>Red = heavy concentration / larger bonded contamination</div><div class="heatItem"><span class="heatSwatch" style="background:#ff8c00"></span>Orange = oil/grease indicators or moderate clusters</div><div class="heatItem"><span class="heatSwatch" style="background:#ffe600"></span>Yellow = lighter contamination or smaller dust particles; Purple = possible grit / sand</div></div>${heatText}</div>
...
(function(){
  ...
  if (help) help.addEventListener('click', () => show('Help', `
    ...
    <h3>Privacy & image handling</h3><p>JPEG test images are processed locally in your browser during this beta. The app does not intentionally upload, store or transmit your original JPEG image or embedded GPS/EXIF metadata. Exported reports, annotated images and clean-preview files are only saved if you choose to download/save them locally. Please avoid using personal or sensitive photographs for testing.</p><h3>Oil / grease / lubricant detection</h3><p>The beta now also looks for circular halo or ring-type marks which can be consistent with oil, grease or shutter/mirror lubricant contamination. This is indicative only and physical inspection is recommended for confirmation.</p><h3>Hard grit / sand detection</h3><p>The beta also looks for larger, high-contrast irregular particles that may be consistent with grit, sand or granular debris. These can be abrasive, so contact cleaning should be avoided unless inspected professionally.</p><h3>Detection mark colours</h3><p>The Auto Detect overlay can be shown in red, cyan/blue or magenta. Cyan/blue has been included as a more accessible option for users who may struggle to see red markings clearly.</p><h3>Need a clean?</h3>
...
(function(){
  ...
  document.querySelectorAll('.colourChoice').forEach(btn => {
    btn.addEventListener('click', function(){
      selectedHighlightColour = this.dataset.colour || '#ff3030';
      if(hidden) hidden.value = selectedHighlightColour;
